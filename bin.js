...
download.getFastRegistry(function (err, fastRegistry) {
  if (err) {
...
function runMain () {
  ...
  const startDownload = function (downloadUrl) {
    const prebuildGithub = require('./package.json')['prebuild-github']
    /** 需要进行替换 */
    Object.entries(prebuildGithub).forEach(([key, value]) => {
    ...
    download(downloadUrl, opts, function (err) {
      ...
      log.info(log.logColors.silly('install Successfully installed prebuilt binary!'))
    })
  }
